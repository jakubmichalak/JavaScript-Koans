...
test("splice", function() {
    ...
    var workingWeek = daysOfWeek.splice(0, 5); //zabieramy elementy z tablicy days of week, startujemy na 0 i konczymy 5 elementow pozniej;
...
test("stack methods", function() {
    var stack = [];
    stack.push("first");
    stack.push("second"); // powstala tablica first, second

    equal("second", stack.pop(), 'what will be the first value popped off the stack?'); // pop usuwa ostatni element
    equal("first", stack.pop(), 'what will be the second value popped off the stack?');// pop usuwa ostatni element
});

test("queue methods", function() {
    var queue = [];
    queue.push("first"); // wpychamy na koniec tablicy first
    queue.push("second"); // wpychamy na koniec tablicy second
    queue.unshift("third"); //wrzucamy na poczatek third to robi funkcja unshift

    equal("third", queue.shift(), 'what will be shifted out first?'); // shift sciaga 1 element z tablocy
